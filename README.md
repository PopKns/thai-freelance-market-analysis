...
ML prototype ที่ marketplace ต้องการจริง (คัดกรองความน่าเชื่อถือ/ความผิดปกติ) — มี tests + CI ครบ

ออกแบบมาเพื่อ 3 กลุ่มผู้ใช้:
...
- **ทีม marketplace** — prototype คัดกรอง fake-review / ความผิดปกติ
...
(สาธารณะ)    จำกัด rate     normalize     ชาร์ต + สถิติ                  หน้าเว็บสาธารณะ
             + เช็ค robots   + anonymize        │
                                                └──▶ src/ml/ (คัดกรอง fraud / ความผิดปกติ)
```
...
- **Representative sampling** — seeded random sample กระจายทั่ว 222k URL จาก products sitemap
- **Reproducible & config-driven** — แยก stage รันเดี่ยวได้, ตั้งค่าผ่าน `config.json`
- **Data quality gates** — `pytest` (synthetic fixtures, รัน offline), `ruff`, GitHub Actions CI
...
python src/clean.py                     # -> data/listings.parquet (anonymized)

streamlit run dashboard/app.py          # 4 แท็บ: pricing advisor · demand map · opportunity finder · trust/anomaly (+ sidebar filters)
python src/ml/anomaly.py                 # คัดกรอง fraud/ความผิดปกติ — จัดอันดับ gig ที่ควรตรวจ + เหตุผล
...
python -m pytest tests/ -q           # 15 tests, รันได้แบบ offline ล้วน (ไม่ต้องมีข้อมูลที่ scrape)
```
...
ครบ Phase 1–3: เก็บข้อมูล → วิเคราะห์ & dashboard → ML คัดกรองความน่าเชื่อถือ/ความผิดปกติ
ชุดข้อมูล ~2,900 gig; 15 tests, CI เขียว — แผนงาน & ผลลัพธ์ดูที่ [`BLUEPRINT.md`](./BLUEPRINT.md)
